...
def train_bert(config: PipeLineConfig):
    ...
    perfect_output = torch.tensor(
        np.hstack([enriched.target.values[:, None], y_aux_train, subgroup_target])
    ).float()
    ...
    torch.save(model.state_dict(), f"./models/final-pipe5-{config.expname}.bin")


if __name__ == "__main__":
    config_1 = PipeLineConfig(
        expname="wiki_cased_1",
        lr=4e-5,
        warmup=0.05,
        epochs=2,
        seed=10400,
